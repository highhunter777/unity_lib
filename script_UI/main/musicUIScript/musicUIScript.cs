...
public class musicUIScript : MonoBehaviour
{
    // Start is called before the first frame update
    void Start()
    {
        if (FindObjectsOfType<musicUIScript>().Length > 1)
        {
    ...
    {
        //获取音量滑动条
        Slider musicSlider = GameObject.FindGameObjectWithTag("musicUI").transform.GetChild(0).GetChild(1).GetChild(1).gameObject.transform.GetComponent<Slider>();
